...
int main(){
  ...
  Serial.println("sx1278 module and STM32F042: send data received from serial and use ack!");
	...
	while(true){
		...
		while(true){
			...
			data_idx++;
			if(ch == '\n') break;
			else if(data_idx == data_sz - 1) break;
		}
	...
	#else
  // Print a start message
  Serial.println("sx1278 module and STM32F042: receiving packets");

	while(true){
	  // Receive message for 10 seconds
	  e = sx1278.receivePacketTimeoutACK(10000);
	  if (e == 0) {
	    ...
	    Serial.print("Package received ERROR\n"); // TODO: timeout olursa ledlere gerek yok!!
